...
class SyncWrapper {
  ...
  start() {
    return (dispatch,getState)=>{
      ...
      let lastRun = null;
      console.log("Starting run with summary", newSummary);

      let cb = async (e, txns, meta) => {
        if(e) {
          console.log("Problem in sync run", e);
          newSummary.errors++;
          return;
        }
        ...
        if(txns) {
          ...
          txns.forEach(t=>{
            ...
            newSummary.totalSize += JSON.stringify(t).length;
            Object.keys(t.logEvents).forEach(k=>{
              let set = t.logEvents[k];
              newSummary.eventCount += set.length;
            })
          });
          let newRun = {
            ...this.run,
          ...
          try {
            ...
            console.log("problem storing data", e);
          }
          */

          console.log("Storage complete, dispatching update");
      ...
      this.runner.start(cb).then(async ()=>{
        console.log("Storing run", lastRun);
        if(lastRun) {
