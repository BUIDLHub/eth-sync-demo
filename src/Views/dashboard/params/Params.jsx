...
export default class Params extends React.Component {
  render() {
    ...
    let needsSnaps = paramData['useSnapshots'] || false;
    let useSnapshots = (
      <input type="checkbox"
             className={cn("ml-2", align.noMarginPad)}
             checked={needsSnaps}
             disabled={!canUseSnaps}
             onChange={e=>this.props.update(paramData, 'useSnapshots', e.target.checked)} />
    )
    let start = paramData['rangeStart'];
    let totalBlocks = paramData['rangeEnd'] - start;
    let updateStart = (blocks) => {
      let start = paramData['rangeEnd'] - 0;
      this.props.update(paramData, "rangeStart", start - (blocks-0));
    }
    let blockRange = (
      <Row className={cn("ml-2", align.full, align.topLeft, align.noMarginPad)}>
        <Col md="8" className={cn(align.leftCenter, align.noMarginPad)}>
          <input type="number"
              className={cn(align.noMarginPad)}
              value={totalBlocks}
              onChange={e=>updateStart(e.target.value-0)}
          />
          <span className={cn("ml-2", "text-left", "text-muted", "font-weight-light", "text-1")}>
            blocks
          </span>
        </Col>
        <Col md="8" className={cn(align.leftCenter, align.noMarginPad)}>
    ...
    return (
      <Row className={cn(align.topCenter, align.full, align.noMarginPad)}>
        ...
        <Col md="10" className={cn(align.leftCenter, "mb-3", align.noMarginPad)}>
          {
            buildRow("Use BUIDLHub Snapshot", useSnapshots)
          }
        </Col>
        <Col md="10" className={cn(align.leftCenter, "mb-3", align.noMarginPad)}>
          {
